...
def test_get_node_officer_found(api_client):
    ...
    """
    response = api_client.get("/nodes/officer/12000001")
...
def test_get_node_entity_found(api_client):
    ...
    """
    response = api_client.get("/nodes/entity/11000001")
    assert response.status_code == 200
    res = response.json()
    assert res["count"] == 1
...
def test_get_node_not_found(api_client):
    ...
    """
    response = api_client.get("/nodes/officer/99999999")
...
def test_get_node_invalid_type_is_empty(api_client):
    """
    Test getting a node with invalid type.
    Now just returns 0 count because we query 'nodes' table where node_type = ?.
    """
    response = api_client.get("/nodes/invalidtype/123")
    assert response.status_code == 200
    res = response.json()
    assert res["count"] == 0
